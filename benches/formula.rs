...
fn simple_formula(input: &[[f32; 4]]) -> Vec<f32> {
    ...
    input.iter().map(|inp| {
        black_box(formula.calculate(inp))
    }).collect()
}
...
fn simple_meval(input: &[[f32; 4]]) -> Vec<f32> {
    ...
    input.iter().map(|inp| {
        black_box(formula(inp[0] as f64, inp[1] as f64, inp[2] as f64, inp[3] as f64)) as f32
    }).collect()
}
...
fn complex_native(input: &[[f32; 4]]) -> Vec<f32> {
    input.iter().map(|[a, b, c, d]| {
        black_box((a.powi(4) / b.sqrt() + c.powi(-1).ln() - d.powi(2)).sqrt())
    }).collect()
}
...
fn complex_formula(input: &[[f32; 4]]) -> Vec<f32> {
    ...
    let formula = Formula::new(vec![
        /*0*/ FormulaNode::Operation(OpNode::Unary(UnaryOp::Sqrt, 1)), // (a.powi(4) / b.sqrt() + (c^-1).ln() - d.powi(2)).SQRT()
        /*1*/ FormulaNode::Operation(OpNode::Binary(BinaryOp::Sub, 2, 3)), // a.powi(4) / b.sqrt() + (c^-1).ln() SUB d.powi(2)
        /*2*/ FormulaNode::Operation(OpNode::Binary(BinaryOp::Add, 4, 5)), // a.powi(4) / b.sqrt() ADD (c^-1).ln()
        /*3*/ FormulaNode::Operation(OpNode::Unary(UnaryOp::Sqr, 6)), // d.POWI(2)
        /*4*/ FormulaNode::Operation(OpNode::Binary(BinaryOp::Div, 7, 8)), // a.powi(4) DIV b.sqrt()
...
fn complex_meval(input: &[[f32; 4]]) -> Vec<f32> {
    ...
    let expr: Expr = "sqrt(a*a*a*a / sqrt(b) + ln(1/c) - d*d)".parse().unwrap();
    let formula = expr.bind4("a", "b", "c", "d").unwrap();
    input.iter().map(|inp| {
        black_box(formula(inp[0] as f64, inp[1] as f64, inp[2] as f64, inp[3] as f64)) as f32
    }).collect()
...
fn complex_evalexprjit(input: &[[f32; 4]]) -> Vec<f32> {
    ...
    let eq = Equation::new("sqrt(a*a*a*a / sqrt(b) + ln(1/c) - d*d)".to_string()).unwrap();
    let mut data = vec![0.; 4];
    input.iter().map(|inp| {
        data[0] = inp[0] as f64;
        data[1] = inp[1] as f64;
...
fn benchmark_simple(c: &mut Criterion) {
    ...
    let input: [[f32; 4]; SIMPLE_N] = std::array::from_fn(|i| {
        [1000. - i as f32 * 4., i as f32 / 2., ((i + 1) as f32 * 153.).sqrt(), 44. + (i as f32 * 0.12)]
    });

    {
        ...
        let compare = |v1: &[f32], v2: &[f32]| {
            ...
            v1.iter().zip(v2.iter()).for_each(|(v1, v2)| if (v1 - v2).abs() > EPS {
                panic!("{v1} {v2}");
            });
        };
    ...
    test("simple-formula", &simple_formula);
...
fn benchmark_complex(c: &mut Criterion) {
    ...
    test("complex-formula", &complex_formula);
