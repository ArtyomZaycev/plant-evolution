...
fn engine_benchmark(engine: &mut Engine, autoevolve_at: u32, total_evolution: u32) {
    engine
...
fn bench_short(c: &mut Criterion) {
    ...
    group
        ...
        .throughput(Throughput::Elements(throughput as u64));
    ...
    group.bench_function("accuracy", |b| {
        engine
            ...
            .unwrap();
        b.iter(|| {
            engine_benchmark(&mut engine, 500, throughput);
        });
    });
...
criterion_group!(benches, bench_short);
