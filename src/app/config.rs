...
impl Config {
    ...
    fn config_dir() -> PathBuf {
        ProjectDirs::from("xyz", "Aspid", "Plant Evolution").map(|project_dirs| {
            project_dirs.config_dir().to_path_buf()
        }).unwrap_or("./config/".into()).with_trailing_sep().to_path_buf()
    }
    ...
    pub fn load() -> Self {
        ...
        let locked_seed = if cfg!(feature = "stable_rng") {
            ...
            fs::read_to_string(dir.with_file_name(Self::LOCKED_SEED_FILE_NAME)).ok().and_then(|v| v.parse::<u64>().ok())
        };
