...
impl MapsTrail {
    ...
    pub fn push(&mut self, map: &MapData, total_evolutions: u32) {
        if self.enabled {
            self.trail
                .push(TrailData::new(map.clone(), total_evolutions));
        }
    }
    ...
    pub fn show(&mut self, ui: &mut egui::Ui, visual_settings: &VisualSettings) -> egui::Response {
        if self.trail.is_empty() {
            ...
            if let Some((index, position)) = self.show_map.clone() {
                ...
                let response = egui::Window::new(format!("Plant 1 №{}", index + 1))
                    ...
                    .show(ui.ctx(), |ui| {
                        ...
                        let border_size = 1.;
                        let cell_size = ((ui.available_width() - border_size * 2.)
                            / MAP_SIZE.0 as f32)
                ...
                let path_rect = Rect::from_min_max(
                    ...
                    response.rect.right_bottom() + Vec2::splat(window_padding),
                );
            ...
            ui.label(format!(
                "Memory usage: {} KB",
                (size_of::<Self>() + self.trail.len() * size_of::<TrailData>()) / 1000
            ));
