...
impl PlantEvolutionApp {
    ...
    fn show_left_panel_content(&mut self, ui: &mut egui::Ui) {
        let response =
            ui.add(TextEdit::singleline(&mut self.selected_map_index_str).desired_width(150.));
        let new_selected_map_index =
            Self::get_selected_map_index_from_str(&self.selected_map_index_str, self.maps.len());
                ...
                .for_each(|(i, cell)| {
                    ui.horizontal_top(|ui| {
                        CollapsingHeader::new(format!(
                            "Cell {} (v={:.2})",
                            i + 1,
                            self.maps[map_idx].evolution_data.cells_evolution_data[i].volatility
                        ))
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.label(format!(
                                "Sunlight (v={:.2}): {:.2}",
                                cell.sunlight_consumption.volatility, *cell.sunlight_consumption
                            ));
                            ui.label(format!(
                                "Air (v={:.2}): {:.2}",
                            ...
                            ui.label(format!(
                                "Minerals (v={:.2}): {:.2}",
                                cell.minerals_consumption.volatility, *cell.minerals_consumption
                            ));
                            ui.label(format!(
                                "Water (v={:.2}): {:.2}",
    ...
    fn show_central_panel_content(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Frame::canvas(ui.style()).show(ui, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                let columns =
                    (((available.x - min_border_size) / (min_map_width + min_border_size)).floor()
                        as usize)
                        .min(self.selected_maps_index.len())
                        .max(1);
                let rows = self.selected_maps_index.len().div_ceil(columns);
                let map_width =
                    (available.x - (columns + 1) as f32 * min_border_size) / columns as f32;
                if self.selected_maps_index.len() == 1 {
                ...
                for i in 0..self.selected_maps_index.len() {
                    ...
                    let canvas_start = Pos2::new(
                        start_pos.x
                            + border_width
                            + column as f32 * map_width
                            + border_width * column as f32,
                    ...
                    if canvas_reponse.hovered()
                        && ui
                            .input(|inp| inp.pointer.hover_pos())
                            .is_some_and(|p| map_rect.contains(p))
                    {
