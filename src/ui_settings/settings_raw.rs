...
use super::{
    evolution_parameters_raw::*,
    saving_parameters_raw::*,
    ui_settings_raw::*,
};
...
impl RawSetting<(UiSettings, EngineParameters)> for SettingsRaw {
    fn new(settings: (UiSettings, EngineParameters)) -> Self {
        Self {
            ...
            evolution_parameters: EvolutionParametersRaw::new((settings.1.evolution_parameters, settings.1.run_evolution_parameters)),
        }
    }
    ...
    fn parse(&self) -> Option<(UiSettings, EngineParameters)> {
        let (evolution_parameters, run_evolution_parameters) = self.evolution_parameters.parse()?;
        Some((
    ...
    fn ui(self, ui: &mut egui::Ui) -> egui::Response {
        ui.add_enabled_ui(matches!(self.state, SettingsRawState::InProgress), |ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::TOP), |ui| {
                if ui.add_enabled(self.is_valid(), egui::Button::new("Apply")).clicked() {
